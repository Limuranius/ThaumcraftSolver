...
for aspect in aspects_list:
    ...
    hist_img = cv2.calcHist([sprite_img], [0, 1, 2], None, [256, 256, 256], [0, 256, 0, 256, 0, 256])
...
def classify_hex(hex_img: np.ndarray) -> int:
    ...
    if ring_score > 400 and inner_circle_score < 150:
        return FREE
    elif ring_score < 200:
        return WALL

    differences = []
    hex_hist = cv2.calcHist([hex_img], [0, 1, 2], None, [256, 256, 256], [0, 256, 0, 256, 0, 256])
...
def get_inner_circle_score(hex_img):
    ...
    # cv2.imshow("Inner circle score", masked)
    # print("Inner circle score:", inner_circle_score)
